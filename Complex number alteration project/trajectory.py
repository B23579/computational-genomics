...
# combinations will be obtained.
        ...
        while stop==False and compt<100:
            ...
            if (Max>final1+1 or Min > final2+1):
                if zero ==False:
                    if nA>nB and (nB,nA) not in trajectory and (nA,nB) not in trajectory:
                        trajectory.append((nB,nA))
                    elif nA<nB and (nA,nB) not in trajectory and (nB,nA) not in trajectory:
            ...
            if element == (nA,nB) and trajectory[-1][0]:
                ...
                element[1]<=2*Temp[1] and element!=trajectory[-1] and element not in trajectory and (element[1],element[0]) not in trajectory ):
                    trajectory.append(element)
                    break
            elif(element[0]>=(Temp[0]-1) and element[1]>=(Temp[1]-1) and element[0]<=2*Temp[0] and 
                element[1]<=2*Temp[1] and element!=trajectory[-1] and element not in trajectory and (element[1],element[0]) not in trajectory ):
...
def Write(trajectory:list,conf)->None:
    ...
    with open(f'json/json_trajectory{conf}.json', 'w') as outfile:
...
if __name__ == "__main__":
    T=Trajectory(0,3, 900000) # issue, when trying to generate complex state with configuration na, na, this will probably fail to generate trajectory, to solve it,  use the funtion extract, read the notice  
    n= len(T)
    print(n)
    ...
    # # # Use the following commented portion of code for complex state na,na
    # Trj=extract( "3:3",Trj,33)
    # n= len(Trj)
    # print(n)
    # print("\n")
    # for i in range(n):
